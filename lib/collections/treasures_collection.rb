class TreasuresCollection
  ...
  def self.save
    @@treasures.each { |treasure| 
      ...
      file.puts("#{treasure.id}||#{treasure.owner.first_name}||#{treasure.type.name}||#{treasure.title}||#{treasure.description}||#{treasure.trashed}||#{treasure.storage.id}||#{treasure.storage.name}||#{treasure.special_note}")
      file.close
    }
  end
  ...
  def self.load(file_name)
    ...
    Dir["#{file_name}/*.treasure"].each do |treasure_file_path|
      treasure_file = File.open(treasure_file_path, "r")
      treasure_array = treasure_file.read.split("||")
      treasure = Treasure.new(treasure_array[1], treasure_array[2], treasure_array[3], treasure_array[4])
      self.add(treasure)
    end
  end
end
